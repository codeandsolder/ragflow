...
class CommonService:
    ...
    def filter_scope_list(cls, in_key, in_filters_list, filters=None, cols=None):
        ...
        if not filters:
            filters = []
        if cols:
            query_records = cls.model.select(*cols).where(getattr(cls.model, in_key).in_(in_filters_list), *filters)
        else:
            query_records = cls.model.select().where(getattr(cls.model, in_key).in_(in_filters_list), *filters)
        return list(query_records)
