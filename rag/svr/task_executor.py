...
async def _init_embedding_model(task):
...
async def _handle_raptor_task(task, progress_callback, embedding_model, vector_size):
...
async def _handle_graphrag_task(task, progress_callback, embedding_model, vector_size):
    kb_task_llm_id = task["kb_parser_config"].get("llm_id") or task["llm_id"]
    task_tenant_id = task["tenant_id"]
    task_language = task["language"]
    task_dataset_id = task["kb_id"]
...
async def _handle_standard_task(task, progress_callback, embedding_model):
    task_document_name = task["name"]
    task_parser_config = task["parser_config"]

    task["llm_id"] = task["parser_config"].get("llm_id") or task["llm_id"]
    start_ts = timer()
    ...
    return chunks, token_count, toc_thread


async def _index_chunks(task, chunks, token_count, progress_callback, toc_thread=None):
    task_id = task["id"]
    task_tenant_id = task["tenant_id"]
    task_dataset_id = task["kb_id"]
...
async def _cleanup_on_cancel(task, exception=None):
    task_id = task["id"]
    task_tenant_id = task["tenant_id"]
    task_dataset_id = task["kb_id"]
    task_doc_id = task["doc_id"]
...
async def do_handle_task(task):
    ...
    try:
        ...
        embedding_model, vector_size = await _init_embedding_model(task)
        ...
        if task_type == "raptor":
            result = await _handle_raptor_task(task, progress_callback, embedding_model, vector_size)
            if result is None:
                return
            chunks, token_count = result
        elif task_type == "graphrag":
            await _handle_graphrag_task(task, progress_callback, embedding_model, vector_size)
            return
        elif task_type == "mindmap":
            progress_callback(1, "place holder")
            return
        else:
            chunks, token_count, toc_thread = await _handle_standard_task(task, progress_callback, embedding_model)
            if chunks is None:
                return

        await _index_chunks(task, chunks, token_count, progress_callback, toc_thread)
        ...
        await _cleanup_on_cancel(task, e)
        raise
