...
def step_04_set_dataset_settings(
    ...
    with step("save dataset settings and assert update payload"):
        ...
        if isinstance(response_payload, dict):
            assert response_payload.get("code") == 0, f"/v1/kb/update response code={response_payload.get('code')} message={response_payload.get('message')}"

        payload = get_request_json_payload(response)
        assert payload.get("kb_id") == dataset_id, f"Expected kb_id={dataset_id!r}, got {payload.get('kb_id')!r}"
