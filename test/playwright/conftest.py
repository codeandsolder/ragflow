...
def _ensure_model_provider_ready_via_api(base_url: str, auth_header: str) -> dict:
    ...
    _, tenant_payload = _api_request_json(_build_url(base_url, "/v1/user/tenant_info"), headers=headers)
...
def _ensure_dataset_ready_via_api(base_url: str, auth_header: str, dataset_name: str) -> dict:
    ...
    _, list_payload_after = _api_request_json(list_url, method="POST", payload={}, headers=headers)
    existing_after = _find_dataset_by_name(list_payload_after, dataset_name)
    if not existing_after:
        raise RuntimeError(f"Dataset {dataset_name!r} not found after kb/create response={create_payload}")
    return {
