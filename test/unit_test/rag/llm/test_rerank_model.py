...
try:
    ...
    _RERANK_MODEL_IMPORT_OK = True
except (ImportError, ModuleNotFoundError, SyntaxError, AttributeError):
    _RERANK_MODEL_IMPORT_OK = False
    pytestmark = pytest.mark.skip(reason="rag.llm.rerank_model cannot be imported due to dependency issues")


class TestRerankModelFactoryRegistration:
    def test_jina_rerank_registered(self):
        assert "Jina" in RerankModel

    def test_xinference_rerank_registered(self):
        assert "Xinference" in RerankModel

    def test_cohere_rerank_registered(self):
        assert "Cohere" in RerankModel
...
class TestBaseRerankModel:
    ...
    def test_normalize_rank_all_same(self):
        ...
        normalized = Base._normalize_rank(ranks)
...
class TestJinaRerank:
    def test_jina_rerank_factory_name(self):
        assert JinaRerank._FACTORY_NAME == "Jina"

    def test_jina_rerank_default_url(self):
        model = JinaRerank("key", "jina-reranker-v2-base-multilingual")
...
class TestXInferenceRerank:
    def test_xinference_rerank_factory_name(self):
        assert XInferenceRerank._FACTORY_NAME == "Xinference"
    ...
    def test_xinference_rerank_appends_v1_rerank(self, mock_post):
        ...
        model = XInferenceRerank("key", "model", base_url="http://localhost:9999")
...
class TestLocalAIRerank:
    def test_localai_rerank_factory_name(self):
        assert LocalAIRerank._FACTORY_NAME == "LocalAI"
    ...
    def test_localai_rerank_normalizes_output(self, mock_post):
        ...
        model = LocalAIRerank("key", "model", base_url="http://localhost:8080")
        rank, tokens = model.similarity("query", ["doc1", "doc2", "doc3"])
...
class TestLmStudioRerank:
    def test_lmstudio_rerank_factory_name(self):
        assert LmStudioRerank._FACTORY_NAME == "LM-Studio"

    def test_lmstudio_rerank_raises_not_implemented(self):
        model = LmStudioRerank("key", "model", base_url="http://localhost:1234")
        with pytest.raises(NotImplementedError):
            model.similarity("query", ["doc1"])
...
class TestOpenAI_APIRerank:
    def test_openai_api_rerank_factory_name(self):
        assert OpenAI_APIRerank._FACTORY_NAME == "OpenAI-API-Compatible"

    def test_openai_api_rerank_normalizes_url(self):
        model = OpenAI_APIRerank("key", "model", base_url="http://localhost:8080/v1")
    ...
    def test_openai_api_rerank_normalizes_output(self, mock_post):
        ...
        model = OpenAI_APIRerank("key", "model", base_url="http://localhost:8080")
        rank, tokens = model.similarity("query", ["doc1", "doc2", "doc3"])
...
class TestTogetherAIRerank:
    def test_togetherai_rerank_factory_name(self):
        assert TogetherAIRerank._FACTORY_NAME == "TogetherAI"

    def test_togetherai_rerank_raises_not_implemented(self):
        model = TogetherAIRerank("key", "model", base_url="http://localhost:8080")
        with pytest.raises(NotImplementedError):
            model.similarity("query", ["doc1"])
...
class TestVoyageRerank:
    @pytest.mark.skip(reason="voyageai module not available")
    def test_voyage_rerank_factory_name(self):
        assert VoyageRerank._FACTORY_NAME == "Voyage AI"

    @pytest.mark.skip(reason="voyageai module not available")
    @patch("rag.llm.rerank_model.voyageai.Client")
    def test_voyage_rerank_init(self, mock_voyage):
        model = VoyageRerank("key", "voyage-rerank-2")
...
class TestGPUStackRerank:
    ...
    def test_gpustack_rerank_similarity(self, mock_post):
        ...
        model = GPUStackRerank("key", "model", base_url="http://localhost:8080")
        rank, tokens = model.similarity("query", ["doc1", "doc2"])
...
class TestRerankModelCircuitBreaker:
    def test_circuit_breaker_uses_configurable_failure_threshold(self):
        model = JinaRerank("key", "model", failure_threshold=10)
        assert model.failure_threshold == 10

    def test_circuit_breaker_uses_configurable_recovery_timeout(self):
        model = JinaRerank("key", "model", recovery_timeout=60)
        assert model.recovery_timeout == 60

    def test_circuit_breaker_uses_default_failure_threshold(self):
        model = JinaRerank("key", "model")
        assert model.failure_threshold == 5

    def test_circuit_breaker_uses_default_recovery_timeout(self):
        model = JinaRerank("key", "model")
        assert model.recovery_timeout == 30

    def test_circuit_breaker_property_returns_breaker(self):
        model = JinaRerank("key", "model")
        breaker = model.circuit_breaker
        assert breaker is not None

    def test_circuit_breaker_per_provider_isolation(self):
        model1 = JinaRerank("key", "model")
        model2 = SILICONFLOWRerank("key", "model")
