...
import pytest
from common import bulk_upload_documents, list_chunks, list_documents, parse_documents, retrieval_chunks
...
def validate_chunks_created(auth, kb_id, document_ids):
...
def validate_chunks_searchable(auth, kb_id, document_ids):
    # Test basic search functionality on chunks
    ...
    if chunks:
        ...
        query = "test"  # Simple query that should match most chunks
        ...
        assert len(res["data"]["hits"]) >= 0, "Retrieval should return at least empty results"


def validate_chunks_chat_capable(auth, kb_id, document_ids):
    # Test that chunks can be used in chat context
    res = list_chunks(auth, {"kb_id": kb_id})
    assert res["code"] == 0, f"Failed to list chunks: {res}"
    chunks = res["data"]["chunks"]
    ...
    if chunks:
        ...
        chunk_content = sample_chunk["content"]
        assert isinstance(chunk_content, str), "Chunk content should be a string"
        assert len(chunk_content) > 0, "Chunk content should not be empty"

        # Test that chunks have proper metadata
        assert "doc_id" in sample_chunk, "Chunk should have doc_id"
        assert "chunk_id" in sample_chunk, "Chunk should have chunk_id"
        assert "weight" in sample_chunk, "Chunk should have weight"
...
class TestDocumentsParse:
    ...
    def test_basic_scenarios(self, WebApiAuth, add_documents_func, payload, expected_code, expected_message):
        ...
        if expected_code == 0:
            ...
            validate_chunks_created(WebApiAuth, kb_id, payload["doc_ids"])
            validate_chunks_searchable(WebApiAuth, kb_id, payload["doc_ids"])
            validate_chunks_chat_capable(WebApiAuth, kb_id, payload["doc_ids"])
        else:
            assert res["message"] == expected_message, res
...
def test_parse_100_files(WebApiAuth, add_dataset_func, tmp_path):
    ...
    validate_document_parse_done(WebApiAuth, kb_id, document_ids)
    validate_chunks_created(WebApiAuth, kb_id, document_ids)
    validate_chunks_searchable(WebApiAuth, kb_id, document_ids)
    validate_chunks_chat_capable(WebApiAuth, kb_id, document_ids)
