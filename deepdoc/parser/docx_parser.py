...
class RAGFlowDocxParser:
    ...
    def __call__(self, fnm, from_page=0, to_page=100000000):
        ...
        keep_all_paragraphs = from_page == 0 and to_page >= 100000000
        for p in self.doc.paragraphs:
            ...
            for run in p.runs:
                if pn > to_page:
                    break
                if from_page <= pn < to_page and run.text:
                    runs_within_single_paragraph.append(run.text)  # append run.text first

                # wrap page break checker into a static method
                run_xml = run._element.xml
                if "lastRenderedPageBreak" in run_xml or 'w:type="page"' in run_xml:
                    has_page_break = True
                    pn += 1

            # For paged extraction, only keep in-range paragraphs.
            # For full-document parsing, preserve legacy behavior of keeping empty paragraphs.
            if runs_within_single_paragraph or (keep_all_paragraphs and not has_page_break):
