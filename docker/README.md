...
> [!IMPORTANT]
> **Security Notice**: This project uses randomly generated passwords by default. For production deployments, review all password settings and ensure they meet your security requirements.
...
All service passwords are now randomly generated by default using:
...
- `SEEKDB_PASSWORD` - SeekDB
- `MYSQL_PASSWORD` - MySQL
- `MINIO_PASSWORD` - MinIO
- `REDIS_PASSWORD` - Redis

To set custom passwords, either:
1. Edit `.env` and set the desired password: `PASSWORD=your_custom_password`
2. Or generate a new random password: `PASSWORD=$(openssl rand -hex 16)`

### Network Binding

By default, the following services bind to `127.0.0.1` only (accessible from localhost):
- MySQL: port 5455
- MinIO: ports 9000, 9001
- Redis: port 6379

This prevents external access to these services. To expose them to the network, modify the port mappings in `docker-compose-base.yml`.
